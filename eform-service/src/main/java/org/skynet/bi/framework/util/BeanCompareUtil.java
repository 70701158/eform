...
public class BeanCompareUtil {
	...
	public static boolean isEquals(Object source, Object target) {
		...
		if (source instanceof Map) {
			rv = compareMap(source, target, rv);
		} else {
			rv = compareRecursive(source, target);
		}
	...
	private static boolean compareMap(Object source, Object target, boolean rv) {
	 ...
	 * @return
	 */
	private static boolean compareRecursive(Object source, Object target) {
		Class<?> srcClass = source.getClass();
		while (!srcClass.getName().equals(Object.class.getName())) {
			boolean rv = compareOneClass(source, target, srcClass);
			if (!rv) {
				return false;
			} else {
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private static boolean compareOneClass(Object source, Object target, Class<?> srcClass) {
		...
		for (Field field : fields) {
			...
			if (target instanceof Map) {
				...
				for (int i = 0; i < targetList.size(); i++) {
					Object srcObj = sourceList.get(i);
					Object targetObj = targetList.get(i);
					boolean rv = compareRecursive(srcObj, targetObj);
					if (!rv) {
						return false;
					}
				}
